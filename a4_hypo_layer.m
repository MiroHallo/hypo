%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% Grid search of the earthquake hypocenter in 1D layered velocity model
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
VelMod.depth(1:VelMod.nlay) = [veloc_mod(2:VelMod.nlay,1);veloc_mod(VelMod.nlay,1)+10]; % bottom depths of the layers boundaries in km
VelMod.Vp = veloc_mod(1:VelMod.nlay,2); % P-waves velocities in layers in km/s
VelMod.Vs = veloc_mod(1:VelMod.nlay,3); % S-waves velocities in layers in km/s
...
for x = 1 : length(gridX)
    display(['Done: ',num2str(100* (x/length(gridX)),'%4.1f' ),'%'])
    for y = 1 : length(gridY)
        for z = 1 : length(gridZ)
            ...
            for st = 1 : N                
                ...
                [nic,tp_synth(st)] = ray1d(VelMod.depth*1000,VelMod.Vp*1000,gridZ(z)*1000,-xyz(st,3)*1000,DistX*1000,10);
                [nic,ts_synth(st)] = ray1d(VelMod.depth*1000,VelMod.Vs*1000,gridZ(z)*1000,-xyz(st,3)*1000,DistX*1000,10);
            end
...
for st = 1 : N    
    ...
    [nic,tp_synth(st)] = ray1d(VelMod.depth*1000,VelMod.Vp*1000,loc_res(3)*1000,-xyz(st,3)*1000,DistX*1000,10);
    [nic,ts_synth(st)] = ray1d(VelMod.depth*1000,VelMod.Vs*1000,loc_res(3)*1000,-xyz(st,3)*1000,DistX*1000,10);
end
