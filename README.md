...
* **Arrival time errors:** The tool evaluates arrival time uncertainties via **Monte Carlo simulations** within stochastically perturbed 1D media. By drawing random samples from Gaussian distributions for layer thicknesses and elastic moduli (shear and bulk), the code generates a distance-dependent distributions of arrival times. These results are synthesized into a distance-dependent error model using a third-order polynomial fit (σ = p<sub>1</sub> d<sup>3</sup> + p<sub>2</sub> d<sup>2</sup> + p<sub>3</sub> d + p<sub>4</sub>, in which d is distance [km] and σ is arrival time uncertainty [s]), ensuring a rigorous Bayesian mapping of model space uncertainties into the data space.
* **Elevation & Borehole handling:** The tool utilizes a single 1D velocity model for the entire domain. Stations are defined by their geographic coordinates and elevation (supporting negative values for borehole sensors). A user-defined reference elevation marks the top of the velocity model. For stations above this reference elevation, the topmost layer is automatically extrapolated upwards. Conversely, borehole sensors are placed at their precise depth and layer within the 1D velocity model (relative to the reference elevation). This allows for accurate localization even in scenarios where the source is shallower than the receiver (e.g., deep monitoring of shallow induced events).
* **Bayesian grid search & PDF:** The location is determined via a parallelized 3D grid search. For each node, the origin time is analytically optimized as the mean residual of P and S arrivals. The tool evaluates a log-likelihood function using distance-dependent uncertainties σ, including the `-ln(σ)` term to correctly weight observations with different error levels. The final posterior PDF is normalized and accounts for full uncertainty quantification, providing a robust probabilistic estimate of the hypocenter.
* **Solutions:** The tool extracts both the Maximum A Posteriori (MAP) solution (the PDF maximum) and the Posterior Mean (PM) solution (the expected value calculated via weighted average). Both results are automatically converted from the internal Cartesian grid back to WGS84 coordinates.
